...
namespace MCKJ
{
    public partial class frmNICForm : Form
    {
        ...
        private int getNextSerialNo()
        {
            ...
            try
            {
                ...
                SqlCommand Command = new SqlCommand("Select ReceiptNo From tblNICForm Order By  cast(ReceiptNo as INT) desc", Conn);
        ...
        private bool checkSerialNo(string SerialNo)
        {
            ...
            try
            {
                ...
                SqlCommand Command = new SqlCommand("Select ReceiptNo From tblNICForm Where ReceiptNo='" + Convert.ToInt32(SerialNo).ToString("00000") + "'", Conn);
        ...
        private bool validateFields()
        {
            ...
            if (txtFCardNo.Text == "")
            {
                MessageBox.Show("Please fill the Required fields!\nRequired field have yellow back color.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            ...
            try
            {
                if (validateFields())
                {
                    ...
                    if (mode == 0)
                    {
                        ...
                        {
                            int MembersID = Convert.ToInt32(dgvHallBooking.Rows[x].Cells[0].Value.ToString());
                            string Age = dgvHallBooking.Rows[x].Cells[1].Value.ToString();
                            int CardStatus = Convert.ToInt32(dgvHallBooking.Rows[x].Cells[2].Value.ToString());
                            decimal Amount = Convert.ToDecimal(dgvHallBooking.Rows[x].Cells[3].Value.ToString());                                                      
                            TotalAmount += Amount;
                            bool result = Convert.ToBoolean(tblNICFormTableAdapter.Add(receipt.ToString("00000"),Date,FCardNo.ToString("00000"),Head,FName,Orakh,MembersID,Age, CardStatus,Amount,Account,ContactNo));
                            if (result)
                            {
                                rSuccess++;
                            }
                        ...
                        tblTransactionsTableAdapter.InsertTransaction(HRD("Identity Card Fee"), "ADV", receipt, Date, 0, TotalAmount, "Identity Card Fee", "", "");
                        //tblTransactionsTableAdapter.InsertTransaction(Account, "NIC", receipt, Date, TotalAmount, 0, "Identity Card Fee", "", "");
                        ...
                        disable(true);
                        this.tblNICFormTableAdapter.Fill(this.comDataSet.tblNICForm);
                        Text = Header;
                    ...
                    else if (mode == 1)
                    {
                        ...
                        for (int x = 0; x < countt; x++)
                        {
                            ...
                            if (dgvHallBooking.Rows[x].Cells[4].Value != null && dgvHallBooking.Rows[x].Cells[4].Value.ToString() != "")
                            {
                                ...
                                result = Convert.ToBoolean(tblNICFormTableAdapter.upd(receipt.ToString("00000"),Date,FCardNo.ToString("00000"),Head,FName,Orakh,MembersID,Age,CardStatus,Amount,Account,ContactNo,rID));
                                added = false;
                            }
                            else
                            {
                                result = Convert.ToBoolean(tblNICFormTableAdapter.Add(receipt.ToString("00000"),Date,FCardNo.ToString("00000"),Head,FName,Orakh,MembersID,Age,CardStatus,Amount,Account,ContactNo));
                                added = true;
                            }
                        ...
                        tblTransactionsTableAdapter.UpdateTransaction(HRD("Identity Card Fee"), "NIC", Convert.ToInt32(receipt), Date, 0, TotalAmount, "Identity Card Fee", "", "", TransactionID(Convert.ToInt32(receipt)));
                        //tblTransactionsTableAdapter.UpdateTransaction(Account, "NIC", Convert.ToInt32(receipt), Date, TotalAmount, 0, "Identity Card Fee", "", "", TransactionID(Convert.ToInt32(receipt)) + 1);
        ...
        private void dgvHallBooking_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 1)
            {
                try
                {
                    int mID =Convert.ToInt32(dgvHallBooking.CurrentRow.Cells[0].Value);
                    SqlConnection con = new SqlConnection(dbLayer.CON_string);
                    SqlCommand cmd = new SqlCommand("Select DOB from tblFamilyMember WHERE FamilyMemberID = " + mID, con);
                    con.Open();
                    string dob = cmd.ExecuteScalar().ToString();
                    string age = "";
                    if (dob.Length == 4)
                        age = Convert.ToString(DateTime.Now.Year - Convert.ToInt32(dob));
                    else
                    {                        
                        ...
                        dgvHallBooking.CurrentRow.Cells[1].Value= year.ToString() + " yr and " + month.ToString() + " months";
                    }
            ...
            {
                int status = Convert.ToInt32(dgvHallBooking.CurrentRow.Cells[2].Value);
                SqlConnection con = new SqlConnection(dbLayer.CON_string);
                SqlCommand cmd1 = new SqlCommand("Select Amount from tblCardType WHERE ID = " + status, con);
                con.Open();

                dgvHallBooking.CurrentRow.Cells[3].Value = cmd1.ExecuteScalar();
            }
            decimal Amt = 0;
            int countt = 0;
            ...
            for (int x = 0; x < countt; x++)
            {
                if (dgvHallBooking.Rows[x].Cells[3].Value != null && dgvHallBooking.Rows[x].Cells[3].Value.ToString() != "")
                {
        ...
        private void frmHallBoking_Load(object sender, EventArgs e)
        {
            ...
            // TODO: This line of code loads data into the 'comDataSet.tblNICForm' table. You can move, or remove it, as needed.
            this.tblNICFormTableAdapter.Fill(this.comDataSet.tblNICForm);
        ...
        {           
            if (checkSerialNo(ReceiptNo))
            {
                ...
                try
                {
                    ...
                    SqlCommand Command = new SqlCommand("SELECT ReceiptNo,Date, FCardNo, Head, MemberName, Age , CardStatus, Amount, tblNICForm.ID,FName,Orakh,ContactNo,Acc FROM tblNICForm Where ReceiptNo ='" + ReceiptNo + "'", Conn);
                    ...
                    {                       
                        ...
                        while (Reader.Read())
                        {
                            ...
                            for (int x = 4; x < Reader.FieldCount - 4; x++)
                            {
                                string value = Reader.GetValue(x).ToString();
                                string vartype = Reader.GetFieldType(x).ToString();
                                    ...
                                    else
                                    {
                                        if (x == 7)
                                        {
                                            ...
                                            dgvHallBooking.Rows[z].Cells[x - 4].Value = (Convert.ToDecimal(value).ToString("F")).ToString();
                                        }
                                        else
                    ...
                    txtTotal.Text = (Convert.ToDecimal(totalAmount.ToString()).ToString("F")).ToString();
                    totalAmount = 0;
                    Reader.Close();
                }
                catch (Exception ex)
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            if (dataGridView1.Rows.Count != 0)
            {
                mode = 1;
                disable(false);
                ReceiptNo = dataGridView1.CurrentRow.Cells["receiptNoDataGridViewTextBoxColumn"].Value.ToString();
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (dataGridView1.Rows.Count != 0)
                {
                    ...
                    {
                        string recipt = dataGridView1.CurrentRow.Cells["receiptNoDataGridViewTextBoxColumn"].Value.ToString();
                        bool result2 = Convert.ToBoolean(tblNICFormTableAdapter.Del(recipt));
                        if (result2)
                        {
                            lblHeader.Text = Header;
                            tblTransactionsTableAdapter.DeleteTransactions("NIC", Convert.ToInt32(recipt));
                            MessageBox.Show("Successfully deleted!");
                            tblNICFormTableAdapter.Fill(comDataSet.tblNICForm);
                        }
                        else
                            lblHeader.Text = Header;
                    }
        ...
        {
            string id = dataGridView1.CurrentRow.Cells["receiptNoDataGridViewTextBoxColumn"].Value.ToString();
            SqlConnection con = new SqlConnection(Community.DBLayer.con_String);
            con.Open();
            ...
            Reports.NICFoam.rptNICFoam rpt = new MCKJ.Reports.NICFoam.rptNICFoam();

            frm.crystalReportViewer1.ReportSource = rpt;
            rpt.SetDataSource(dt);
            frm.Text = "NIC Form Receipt";
            ...
            if (txtFCardNo.Text != "")
            {
                try
                {
                    ...
                    if (reader.HasRows)
                    {
                        ...
                        reader.Close();
                        usp_SEL_tblFamilyMemberTableAdapter.Fill(comDataSet.usp_SEL_tblFamilyMember, txtFCardNo.Text);
                        uspSELtblFamilyMemberBindingSource.Filter = "Gender = 'Male' and AgeGroup='Adult'";
                    }
                    else
        ...
        private void dgvHallBooking_UserAddedRow(object sender, DataGridViewRowEventArgs e)
        {
            ...
            if (dgvHallBooking.Rows.Count > 20)
            {
                dgvHallBooking.AllowUserToAddRows = false;
            }
            else
